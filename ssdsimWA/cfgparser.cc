...
int ConfigParser_t::readFile(const string &fname) {
    ...
    if (fpvec.back() == NULL)
    {
        //LOG(WARN2, "Cannot open config file '%s' (%s)", fname.c_str(), strerror(errno));
        return -1; /*error*/
    }
    ...
    while (fpvec.size() > 0)
    {
                ...
                && memcmp(ent.name.c_str(), ".include ", 9) == 0)
        {
            ...
            if (fpvec.back() == NULL)
            {
                //LOG(WARN2, "Cannot open included config file '%s' ""(%s)", fname.c_str(), strerror(errno));
                return -1; /*error*/
            }
        }
        else
        {
            ...
            if (!ent.name.empty())
            {
                ...
                //DBG(DBG1, "Config option [%s] '%s' = '%s'", ent.section.c_str(), ent.name.c_str(), ent.value.c_str());
                entry.push_back(ent);
            }
        }
    }
...
int ConfigParser_t::getValue(string section, string name, string *value) const {
    ...
    while (i < entry.size())
    {
        ...
        {
            /* entry found */
            //DBG(DBG1, "Get config value [%s] '%s' = '%s'", section.c_str(), name.c_str(), entry[i].value.c_str());
    ...
    //DBG(DBG1, "Get config value [%s] '%s' FAILED", section.c_str(), name.c_str());
    return 0; /*not found*/
}
